function res = vl_simplenn7(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
      ...
      case 'conv-psi'
          ...
          thisx = res(i).x;
          [L1,L2,M1,N]=size(thisx);
          thisx = reshape(thisx,L1,L2,1,M1*N);
          
          wthisx = l.weights{1};
          K = size(wthisx,4);
          
          bthisx = l.weights{2}; %all zero
          
          nextx = vl_nnconv(thisx, wthisx, bthisx, ...
              'pad', l.pad, ...
              'stride', l.stride, ...
              'dilate', l.dilate, ...
              l.opts{:}, ...
          ...
          [L1next,L2next,M2tmp,Ntmp]=size(nextx);
          
          res(i+1).x = reshape(nextx, L1next,L2next,K*M1,N);
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
        ...
        case 'conv-psi'
            ...
            dzdxnext = res(i+1).dzdx;
            [L1next,L2next,M2,Nnext]=size(dzdxnext);
            
            assert(M2 == K*M1);
            assert(Nnext == N);
            
            dzdxnext = reshape(dzdxnext, L1next,L2next, K, M1*N);
            
            
            [thisdzdx, dzdw1, dzdw2] = ...
                vl_nnconv(thisx, wthisx, bthisx, dzdxnext, ...
                'pad', l.pad, ...
                'stride', l.stride, ...
                'dilate', l.dilate, ...
                l.opts{:}, ...
            ...
            res(i).dzdx = reshape(thisdzdx,L1,L2,M1,N);
            dzdw{1} = dzdw1;
            dzdw{2} = dzdw2;
      ...
      case 'conv-dcf'
        ...
        % project
        psi = l.bases{1};
        invpsi = l.bases{2}; %invpsi
        if ~isempty(invpsi)
            
            sz = size(dzdw{1});
            tmp = reshape(dzdw{1}, [L*L, sz(3)*sz(4)]);
            
            tmp = psi*(invpsi'*tmp);
            dzdw{1} = reshape(tmp, [L,L,sz(3),sz(4)]);
        end
